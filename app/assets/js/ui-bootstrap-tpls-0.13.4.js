        ...
        link: function (scope, element, attrs, ctrls) {
          ...
          element.on("keypress", function (e) {
            if (
              attrs.disabled ||
              e.which !== 32 ||
              $document[0].activeElement !== element[0]
            ) {
              return;
            }
...
angular
  ...
  .controller("CarouselController", [
    ...
    "$interval",
    "$animate",
    function ($scope, $element, $interval, $animate) {
      var self = this,
      ...
      $scope.indexOfSlide = function (slide) {
        return angular.isDefined(slide.index)
          ? +slide.index
...
angular
  ...
  .service("dateParser", [
    ...
    "$locale",
    "orderByFilter",
    function ($log, $locale, orderByFilter) {
      ...
      angular.forEach(["minMode", "maxMode"], function (key) {
        if ($attrs[key]) {
          $scope.$parent.$watch($parse($attrs[key]), function (value) {
            ...
            if (
              (key == "minMode" &&
                self.modes.indexOf($scope.datepickerMode) <
          ...
          ctrl._refreshView = function () {
            ...
            for (var i = 0; i < 12; i++) {
              ...
              months[i] = angular.extend(
                ctrl.createDateObject(date, ctrl.formatMonth),
                {
                  uid: scope.uniqueId + "-" + i,
                }
              );
          ...
          ctrl._refreshView = function () {
            ...
            for (
              ...
              years[i] = angular.extend(
                ctrl.createDateObject(date, ctrl.formatYear),
                {
                  uid: scope.uniqueId + "-" + i,
                }
              );
      ...
      return {
        ...
        link: function (scope, element, attrs, ngModel) {
          var dateFormat,
            ...
            datepickerTemplateUrl = angular.isDefined(
          ...
          scope.isDisabled = function (date) {
            ...
            return (
              (scope.watchData.minDate &&
                scope.compare(date, cache.minDate) < 0) ||
              (scope.watchData.maxDate &&
                scope.compare(date, cache.maxDate) > 0)
            );
          };
          ...
          if (datepickerPopupConfig.html5Types[attrs.type]) {
            ...
            dateFormat =
              attrs.datepickerPopup || datepickerPopupConfig.datepickerPopup;
              ...
              if (attrs[key]) {
                ...
                if (key === "datepickerMode") {
                  ...
                  scope.$watch("watchData." + key, function (value, oldvalue) {
                    if (
                      angular.isFunction(setAttribute) &&
                      value !== oldvalue
                    ) {
      ...
      scope.$watch("isOpen", function (isOpen, wasOpen) {
        if (appendToBody && self.dropdownMenu) {
          ...
          if (!rightalign) {
            ...
            css.right =
              window.innerWidth -
              (pos.left + self.$element.prop("offsetWidth")) +
              "px";
          }
          ...
          modalRenderDeferObj.promise.then(function () {
            ...
            if (attrs.modalInClass) {
              if ($animateCss) {
                ...
                animationPromise = $animate.addClass(
                  element,
                  attrs.modalInClass
                );
              }
              ...
              scope.$on(
                ...
                function (e, setIsAsync) {
                  ...
                  if ($animateCss) {
                    ...
                    $animate
                      .removeClass(element, attrs.modalInClass)
                      .then(done);
                  }
                }
              );
            ...
            $q.when(animationPromise).then(function () {
              var inputsWithAutofocus =
                element[0].querySelectorAll("[autofocus]");
              /**
  ...
  .factory("$modalStack", [
    ...
    function (
      ...
      function removeModalWindow(modalInstance, elementToReceiveFocus) {
        ...
        removeAfterAnimate(
          ...
          function () {
            ...
            body.toggleClass(
              modalBodyClass,
              openedClasses.hasKey(modalBodyClass)
            );
          }
        );
        checkRemoveBackdrop();
      ...
      $modalStack.loadFocusElementList = function (modalWindow) {
        if (
          focusableElementList === undefined ||
          ...
          if (modalWindow) {
            ...
            if (modalDomE1 && modalDomE1.length) {
              focusableElementList =
                modalDomE1[0].querySelectorAll(tababbleSelector);
            }
          }
        }
      };
          ...
          return {
            ...
            compile: function (tElem, tAttrs) {
              ...
              return function link(scope, element, attrs, tooltipCtrl) {
                ...
                var isOpenExp = angular.isDefined(attrs[prefix + "IsOpen"])
                  ...
                  if (!positionTimeout) {
                    positionTimeout = $timeout(
                      function () {
                        ...
                        var ttCss = $position.positionElements(
                          element,
                          tooltip,
                          ttScope.placement,
                          appendToBody
                ...
                var appendToBodyVal = scope.$eval(
    ...
    function ($animate, $sce, $compile, $templateRequest) {
      return {
        link: function (scope, elem, attrs) {
          ...
          scope.$watch(
            $sce.parseAsResourceUrl(attrs.tooltipTemplateTransclude),
            function (src) {
...
angular
  ...
  .controller("ProgressController", [
    ...
    function ($scope, $attrs, progressConfig) {
      ...
      this.bars = [];
      $scope.max = angular.isDefined($scope.max)
        ? $scope.max
        : progressConfig.max;
  ...
  .directive("progress", [
    ...
    function ($log, $progressSuppressWarning) {
      return {
        restrict: "EA",
        replace: true,
        transclude: true,
    ...
    function ($scope, $attrs, ratingConfig) {
      ...
      $scope.rate = function (value) {
        if (!$scope.readonly && value >= 0 && value <= $scope.range.length) {
          ngModelCtrl.$setViewValue(
            ngModelCtrl.$viewValue === value ? 0 : value
          );
          ngModelCtrl.$render();
        }
      };
...
angular
  ...
  .directive("typeaheadPopup", function () {
    return {
      ...
      templateUrl: function (element, attrs) {
        return (
          attrs.popupTemplateUrl || "template/typeahead/typeahead-popup.html"
        );
      },
      link: function (scope, element, attrs) {
